...
1. **네트워킹** 탭에서 사용 가능한 옵션을 검토하고 **공용 액세스를 사용하지 않도록 설정하고 프라이빗 액세스를 사용합니다.** 를 선택합니다.
...
1. **검토**를 선택하고 유효성 검사 프로세스가 완료될 때까지 기다린 다음 **만들기**를 클릭합니다.
...
1. **보안 + 네트워킹** 탭에서 **네트워킹**을 선택합니다. 공용 네트워크 액세스가 사용하지 않도록 설정되었습니다.

    + **공용 액세스 수준**을 **선택한 가상 네트워크 및 IP 주소에서 사용**으로 변경합니다.
    + **방화벽** 섹션에서 **클라이언트 IP 주소 추가** 확인란을 선택합니다.
    + 변경 내용을 **저장**합니다. 
  
1. **데이터 관리** 섹션에서 **중복성** 블레이드를 확인합니다. 기본 및 보조 데이터 센터 위치에 대한 정보를 확인합니다.

1. **데이터 관리** 섹션에서 **수명 주기 관리**를 선택한 다음 **규칙 추가**를 선택합니다.
...
1. **URL** 파일을 복사하여 새 **Inprivate** 탐색 창에 붙여넣습니다.
...
1. Azure Portal에서 스토리지 계정으로 다시 이동하여 **데이터 스토리지** 섹션에서 **파일 공유**를 클릭합니다.

1. **+ 파일 공유**를 클릭하고 **기본 사항** 탭에서 파일 공유 이름을 `share1`로 지정합니다. 

1. **계층** 옵션을 확인합니다. 기본 **트랜잭션 최적화**를 유지합니다.
...
1. 스토리지 계정으로 돌아가서 **Storage Browser**를 선택합니다. Azure Storage Browser는 계정의 모든 스토리지 서비스를 빠르게 볼 수 있는 포털 도구입니다.
...
1. **설정** 섹션에서 **서브넷** 블레이드를 선택합니다.
    + **기본** 서브넷을 선택합니다.
    + **서비스 엔드포인트** 섹션의 **서비스** 드롭다운에서 **Microsoft.Storage**를 선택합니다.
    + 다른 사항은 변경하지 마세요.    
    + 변경 내용을 **저장**합니다. 
...
1. **기존 가상 네트워크 추가**를 선택하고 **vnet1** 및 **기본** 서브넷을 선택한 다음 **추가**를 선택합니다.

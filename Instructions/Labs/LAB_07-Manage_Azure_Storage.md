...
1. **네트워킹** 탭의 **네트워크 액세스** 섹션에서 **공용 액세스 사용 중지 및 개인 액세스 사용**을 선택합니다. 이렇게 하면 아웃바운드 액세스를 허용하는 동안 인바운드 액세스가 제한됩니다. 
...
1. **보안 + 네트워킹** 블레이드에서 **네트워킹**을 선택합니다. **공용 네트워크 액세스**를 사용하지 않도록 설정되었습니다.
    ...
    + **공용 네트워크 액세스**를 **선택한 네트워크 및 IP 주소에서 사용**으로 변경합니다.
...
1. **데이터 관리** 블레이드에서 **수명 주기 관리**를 선택한 다음 **규칙 추가**를 선택합니다.
    ...
    + **기본 Blob** 탭에서 *기본 Blob을 *`30 days` 전에 마지막으로 수정한 경우 *이후***쿨 스토리지로 이동**을 선택합니다. 다른 선택 사항을 확인합니다. 

...
1. **인증 방법** 레이블 옆에 있는 Microsoft Entra 사용자 계정** 링크로 **전환 링크를 클릭합니다.

    > **참고**: 인증 방법을 변경할 때 오류가 표시됩니다(오류는 *"Microsoft Entra와 함께 사용자 계정을 사용하여 데이터를 나열할 수 있는 권한이 없습니다."*). 이것은 예상된 것입니다.  
...
1. 검토 + 할당**을 클릭한 다음 **검토 + 할당**을 클릭하고 **az104-07 컨테이너 컨테이너**의 **개요** 블레이드로 돌아가**서 인증 방법을 변경할 수 있는지 확인합니다(Microsoft Entra 사용자 계정으로 전환).

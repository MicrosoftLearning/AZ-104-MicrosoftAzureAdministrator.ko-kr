...
1. **보안 + 네트워킹** 블레이드에서 **네트워킹**을 선택합니다. **공용 네트워크 액세스**를 사용하지 않도록 설정되었습니다.
    ...
    + **관리**를 선택하고 **공용 네트워크 액세스** 설정을 **사용**으로 변경합니다. 
    + **공용 네트워크 액세스 범위**를 **선택한 네트워크에서 사용**으로 변경합니다.
    + **IPv4 주소** 섹션에서 **클라이언트 IPv4 주소 추가**를 선택합니다.
    + 변경 내용을 저장합니다.
...
1. **데이터 관리** 블레이드에서 **수명 주기 관리**를 선택한 다음 **규칙 추가**를 선택합니다.
    ...
    + 규칙 `Movetocool`의 **이름을 지정합니다**. 규칙 범위를 제한하기 위한 옵션을 확인합니다. **다음**을 클릭합니다. 
    
    + **규칙 추가** 페이지에서 기본 Blob이 마지막으로 수정된 지 `30`일이 넘은 *경우** ***쿨 스토리지로 이동**합니다. 다른 선택 사항을 확인합니다. 
...
1. 업로드 파일을 선택하고 **다운로드**, **삭제**, **계층 변경**, **임대 획득** 등의 줄임표(…) 옵션을 검토합니다.

1. 파일 **URL**(설정 --> 속성 블레이드)을 복사하여 새 **Inprivate** 검색 창에 붙여넣습니다.
...
1. 업로드한 파일을 다시 찾아 오른쪽 끝에 있는 줄임표(...)를 선택한 다음 **SAS 생성**을 선택하고 다음 설정을 지정합니다(다른 설정은 기본값으로 둡니다).
...
1. 포털에서 `Virtual networks`를 검색하여 선택합니다.
...
1. **공용 네트워크 액세스**에서 **관리**를 선택합니다. 

1. **가상 네트워크 추가**를 선택한 다음, **기존 네트워크 추가**를 선택합니다.

1. **vnet1**과 **기본** 서브넷을 선택하고, **추가**를 선택합니다.

1. **IPv4 주소** 섹션에서 머신 IP 주소를 **삭제**합니다. 허용된 트래픽은 가상 네트워크에서만 발생해야 합니다. 
...
1. **스토리지 브라우저**를 선택하고 페이지를 **새로 고침**합니다. 파일 공유 또는 Blob 콘텐츠로 이동합니다.  
    ...
    >**참고:** *이 작업을 수행할 권한이 없음*이라는 메시지가 표시됩니다. 가상 네트워크에서 연결하고 있지 않습니다. 이 내용이 적용되려면 몇 분 정도 걸릴 수 있습니다. 파일 공유는 여전히 볼 수 있지만, 스토리지 계정에 있는 파일이나 Blob은 볼 수 없습니다. 

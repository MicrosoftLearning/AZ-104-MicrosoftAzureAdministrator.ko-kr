...
1. 편집기 창에서 다음과 같이 변경합니다.
    ...
    + **az104_disk1**을 `az102_disk2`로 변경합니다(1개 위치 변경).
...
1. **탑재된 스토리지가 없습니다** 화면에서 **고급 설정 표시**를 선택하고 필요한 정보를 제공합니다. 

    >**참고:** Cloud Shell로 작업할 때 스토리지 계정과 파일 공유가 필요합니다. 

    | 설정 | 값 |
    |  -- | -- |
    | 리소스 그룹 | **az104-rg3** |
    | 스토리지 계정(새로 만들기) | `sacloudshell`(전역적으로 고유해야 하며 길이는 3~24자여야 하며 숫자와 소문자만 사용해야 함) |
    | 파일 공유(새로 만들기) | `fs-cloudshell` |

1. 완료되면 **스토리지 만들기**를 선택합니다. 이 작업은 Cloud Shell을 처음 사용할 때만 수행하면 됩니다. 스토리지를 프로비전하는 데 몇 분 정도 걸립니다.

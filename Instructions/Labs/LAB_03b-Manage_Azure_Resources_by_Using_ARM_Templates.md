...
## 작업 3: Cloud Shell을 구성하고 PowerShell을 사용하여 템플릿 배포 
...
1. **시작** 화면에서 **스토리지 계정 탑재**를 선택하고 **스토리지 계정 구독**을 선택한 다음 **적용**을 선택합니다.

1. **스토리지 계정을 만들려고 합니다**를 선택한 후 **다음**을 선택합니다. **스토리지 계정 만들기** 정보를 완료합니다. 
...
1. **설정**(상단 ​​표시줄)을 선택한 다음 **클래식 버전으로 이동**을 선택합니다.

1. **파일 업로드/다운로드** 아이콘(상단 표시줄)을 선택한 다음 **업로드**를 선택합니다.

1. **다운로드** 디렉터리에서 템플릿과 매개 변수 파일을 모두 업로드합니다. 

1. **편집기(중괄호)** 아이콘을 선택하고 탐색 창 왼쪽에 있는 템플릿 JSON 파일로 이동합니다.

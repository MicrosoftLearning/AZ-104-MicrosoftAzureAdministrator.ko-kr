...
1. **OK**를 클릭하여 정책을 만든 다음 **Virtual Machines** 섹션에서 **Add**(아래로 스크롤)를 선택합니다.
...
1. 다음 정보를 사용하여 스토리지 계정을 정의한 후 **검토 + 만들기**를 선택합니다.
...
1. **모니터링** 블레이드에서 **진단 설정**을 선택한 다음, **진단 설정 추가**를 선택합니다.
...
1. 백업 작업의 **세부 정보를 확인합니다**(오른쪽으로 스크롤하여 링크 보기).
...
1. **다음: 고급 설정**을 선택합니다. 리소스 선택이 완료되었습니다. 

1. 아래로 스크롤하여 자동화 계정을 **만듭니다**. 

   >**참고:** 설정이 채워져야 하며, 그렇지 않으면 유효성 검사에 실패합니다. 

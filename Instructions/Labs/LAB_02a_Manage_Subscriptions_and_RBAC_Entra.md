...
1. **관리 그룹** 블레이드 상단에 있는 메시지를 검토합니다. **디렉터리 관리자로 등록되었지만 루트 관리 그룹에 액세스하는 데 필요한 권한이 없다는** 메시지가 표시되는 경우 다음 단계 시퀀스를 수행합니다.
...
1. Azure Portal **Microsoft Entra ID**를 검색하여 선택하고 **사용자를** 클릭한 다음 **+ 새 사용자를** 클릭합니다.
...
1. Azure Portal **Microsoft Entra ID**를 검색하여 선택하고 **사용자를** 클릭합니다.

...
1. 명령 스크립트** 실행 블레이드에서 **다음을 입력하고 실행을** 클릭하여 **az104-08-vm1**에 **호스트된 웹 사이트에 액세스합니다.
   ...
   ```
    ...
    >**참고**: **-URI** 매개 변수는 VM의 개인 IP 주소**입니다**. **네트워킹** 섹션에서 az104-08-vm1** 블레이드**로 이동하고 네트워크 설정을 클릭합니다 **.**
...
1. 다음 설정을 사용하여 관리 디스크를 만들고(다른 사용자는 기본값을 그대로 유지) 적용**을 클릭합니다**.
...
1. **템플릿** 편집 블레이드의 템플릿 내용을 표시하는 섹션에서 줄 54 **(`"dataDisks": [ ],`)를 다음 코드로 바꿉**니다.
...
1. **가상 머신 확장 집합 만들기** 블레이드의 **네트워킹** 탭에서 **가상 네트워크** 텍스트 상자 아래 **가상 네트워크 만들기** 링크를 클릭하고 다음 설정으로 새 가상 네트워크를 만듭니다(다른 설정은 기본값으로 남겨둡니다). 
...
1. **가상 머신 확장 집합** 블레이드의 **네트워킹** 탭으로 돌아와서 **부하 분산** 섹션 아래에서 다음 설정을 지정합니다(다른 설정은 기본값으로 남겨둡니다).
...
1. **az10408vmss0** 블레이드의 **개요** 섹션에서 인스턴스**를 클릭하고 **가상 머신 확장 집합의 두 인스턴스 옆에 있는 검사 상자를 선택하고 업그레이드**를 클릭한 **다음 확인 메시지가 표시되면 예를** 클릭합니다**.

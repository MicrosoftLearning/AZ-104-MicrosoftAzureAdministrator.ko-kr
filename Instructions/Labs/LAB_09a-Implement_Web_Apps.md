...
1. **배포 센터** 블레이드에서 **로컬 Git/FTPS 자격 증명** 탭을 선택하고, **사용자 범위** 섹션에서 다음 설정을 지정하고 **저장**을 클릭합니다.
    ...
    >**참고:** 이러한 자격 증명을 메모장에 복사합니다. 나중에 필요합니다.
    
    >**참고:** 이러한 자격 증명은 URI를 통해 전달됩니다. URI 해석에 영향을 주는 특수 문자는 포함하지 마세요. 예를 들어 @, $또는 #입니다. 별표 또는 더하기 기호(문자열 중간에)가 작동합니다.
...
1. 스테이징 슬롯 블레이드에서 **개요** 를 클릭한 다음 **기본 도메인** 링크를 클릭하여 새 브라우저 탭에 기본 웹 페이지를 표시합니다.
...
1. 웹앱의 프로덕션 슬롯 블레이드에서 **개요** 를 클릭한 다음 **기본 도메인** 링크를 클릭하여 새 브라우저 탭에 웹 사이트 홈페이지를 표시합니다.
...
1. **자동 크기 조정 설정을** 선택하고 **실행 기록** 탭을 선택합니다. **마지막 데이터 표시**에서 **1시간**을 선택하고 **관찰된 리소스 instance 수를 검사**.

...
조직에서는 회사 웹 사이트를 호스트하기 위해 Azure Web Apps에 관심이 있습니다. 웹 사이트는 현재 온-프레미스 데이터 센터에서 호스트됩니다. 웹 사이트는 PHP 런타임 스택을 사용하여 Windows 서버에서 실행됩니다. 하드웨어의 수명이 거의 끝나가고 있으며 곧 교체해야 합니다. 조직에서는 Azure를 사용하여 웹 사이트를 호스트하여 새 하드웨어 비용을 방지하려고 합니다. 
...
![작업의 다이어그램.](../media/az104-lab09a-architecture.png)
...
이 작업에서는 Azure 웹앱을 만듭니다. Azure 앱 Services는 웹, 모바일 및 기타 웹 기반 애플리케이션을 위한 PAAS(Platform As a Service) 솔루션입니다. Azure 웹앱은 PHP, Java 및 .NET과 같은 대부분의 런타임 환경을 호스팅하는 Azure 앱 서비스의 일부입니다. 선택한 앱 서비스 계획은 웹앱 컴퓨팅, 스토리지 및 기능을 결정합니다. 
...
1. **웹앱 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 유지).
 ...
 1. 검토 + 만들기**를 클릭한 다음 **만들기**를 클릭합니다**.
    ...
    >**참고**: 다음 작업을 진행하기 전에 웹앱이 생성될 때까지 기다립니다. 약 1분 정도 소요됩니다.

1. 배포 후 리소스**로 이동을 선택합니다**.

## 작업 2: 배포 슬롯 만들기 및 구성

이 작업에서는 스테이징 배포 슬롯을 만듭니다. 배포 슬롯을 사용하면 퍼블릭(또는 최종 사용자)이 앱을 사용할 수 있도록 하기 전에 테스트를 수행할 수 있습니다. 테스트를 수행한 후 개발 또는 스테이징에서 프로덕션으로 슬롯을 교환할 수 있습니다. 많은 조직에서 슬롯을 사용하여 사전 프로덕션 테스트를 수행합니다. 또한 많은 조직에서 모든 애플리케이션(예: 개발, QA, 테스트 및 프로덕션)에 대해 여러 슬롯을 실행합니다.

1. 새로 배포된 웹앱의 블레이드에서 기본 실행기본** 링크를 클릭하여 **새 브라우저 탭에 기본 웹 페이지를 표시합니다.

1. 새 브라우저 탭을 닫고 Azure Portal**로 돌아가서 웹앱 블레이드의 배포** 섹션에서 배포 슬롯을** 클릭합니다**.

    >**참고**: 이 시점에서 웹앱에는 PRODUCTION**이라는 레이블이 지정된 단일 배포 슬롯이 **있습니다.
...
이 작업에서는 웹앱 배포 설정을 구성합니다. 배포 설정은 지속적인 배포를 허용합니다. 이렇게 하면 앱 서비스에 최신 버전의 애플리케이션이 있습니다.

1. 스테이징 슬롯에서 배포 센터를** 선택한 **다음, 설정** 선택합니다**.

    >**참고:** 스테이징 슬롯 블레이드(프로덕션 슬롯 대신)에 있는지 확인합니다.
    
1. **원본** 드롭다운 목록에서 외부 Git을** 선택합니다**. 다른 선택 사항을 확인합니다. 
...
>**참고:** 배포에는 1분 정도 걸릴 수 있습니다. 애플리케이션 페이지를 새로 고쳐**야 **합니다.
...
1. 배포 슬롯 블레이드로 **다시 이동한 다음, 교환**을 선택합니다**.**

1. 기본 설정을 검토하고 교환**을 클릭합니다**.

1. 웹앱의 **개요** 블레이드에서 기본 수행기본** 링크를 선택하여 **웹 사이트 홈페이지를 표시합니다.

1. 프로덕션 웹 페이지에 헬로 월드 표시되는지 확인합니다 **.** 페이지에서 사용할 수 있는 바로 가기 키를 보여 줍니다.

    >**참고:** 다음 작업에서 부하 테스트에 필요한 기본 기본 **URL**을 복사합니다. 

## 작업 5: Azure Web App의 자동 크기 조정 구성 및 테스트

이 작업에서는 Azure Web App의 자동 크기 조정을 구성합니다. 자동 크기 조정을 사용하면 웹앱에 대한 트래픽이 증가할 때 웹앱에 대한 최적의 성능을 기본 수 있습니다. 앱의 크기를 조정해야 하는 시기를 확인하려면 CPU 사용량, 메모리 또는 대역폭과 같은 메트릭을 모니터링할 수 있습니다.

1. **설정** 섹션에서 규모 확장(App Service 계획)**을 선택합니다**.

    >**참고:** 스테이징 슬롯이 아닌 프로덕션 슬롯에서 작업하고 있는지 확인합니다.  

1. **크기 조정 섹션에서 자동**을** 선택합니다**. **규칙 기반** 옵션을 확인합니다. 규칙 기반 크기 조정은 다양한 앱 메트릭에 대해 구성할 수 있습니다. 
...
+ [App Service를 확장 및 스케일 아웃하여 수요를 효율적으로 충족하도록 App Service 웹앱을 확장합니다](https://learn.microsoft.com/training/modules/app-service-scale-up-scale-out/). 사용 가능한 리소스를 증분 방식으로 늘려 활동 증가 기간에 대응한 다음, 비용을 절감하고 활동이 감소할 때 이러한 리소스를 줄입니다.

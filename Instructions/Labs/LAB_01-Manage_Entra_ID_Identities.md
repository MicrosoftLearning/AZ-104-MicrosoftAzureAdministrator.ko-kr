...
1. **라이선스**를 선택합니다. 여기에서 라이선스를 구매하고, 보유한 라이선스를 관리하고, 사용자 및 그룹에 라이선스를 할당할 수 있습니다. 사용 가능한 기능을 보려면 **라이선스가 부여된 기능**을 선택합니다.
...
**고유의 구독**으로 작업하는 경우 랩 리소스를 삭제해 보세요. 이렇게 하면 리소스가 확보되고 비용이 최소화됩니다. 랩 리소스를 삭제하려면 랩 리소스 그룹을 삭제하는 것이 가장 쉽습니다. 생성한 Entra ID 사용자 또는 그룹은 제거되지 않습니다. 

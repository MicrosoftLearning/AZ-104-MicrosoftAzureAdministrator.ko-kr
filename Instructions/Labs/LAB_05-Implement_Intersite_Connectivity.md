...
1. 기본 사항 탭에서 다음 정보를 사용하여 양식을 완료한 다음 다음을 선택합니다 **. 디스크 >**. 지정하지 않은 설정의 경우 기본값을 그대로 둡니다.
    ...
    | --- | --- |
    | 구독 |  *구독* |
    | 리소스 그룹 |  `az104-rg5` (필요한 ** 경우새로 만듭니다**. )
...
1. 디스크** 탭에서 **기본값을 사용하고 다음: 네트워킹 >** 선택합니다**.

1. 네트워킹 탭의 **가상 네트워크에 대해 새로** 만들기를 선택합니다**.**

1. 다음 정보를 사용하여 가상 네트워크를 구성한 다음 확인을 선택합니다****. 필요한 경우 기존 정보를 제거하거나 바꿉다.
...
1. 리소스가 생성될 때까지 기다릴 필요가 없습니다. 다음 작업을 계속합니다.
    ...
    >**참고:** 이 작업에서 가상 머신을 만들 때 가상 네트워크를 만든 것을 확인했나요? 가상 네트워크 인프라를 만든 다음 가상 머신을 추가할 수도 있습니다. 
...
이 작업에서는 피어된 가상 네트워크의 리소스가 서로 통신할 수 있는지 확인합니다. Network Watcher는 연결을 테스트하는 데 사용됩니다. 계속하기 전에 두 가상 머신이 모두 배포되고 실행 중인지 확인합니다. 
...
1. **진단 테스트 실행**을 선택합니다.
    ...
    >**참고**: 결과를 반환하는 데 몇 분 정도 걸릴 수 있습니다. 결과가 수집되는 동안 화면 선택이 회색으로 표시됩니다. **커넥트성 테스트**는 UnReachable을 표시**합니다**. 이는 가상 머신이 서로 다른 가상 네트워크에 있기 때문에 의미가 있습니다. 

 
## 작업 4: 가상 네트워크 간 가상 네트워크 피어링 구성

이 작업에서는 가상 네트워크의 리소스 간 통신을 사용하도록 설정하는 가상 네트워크 피어링을 만듭니다. 

1. Azure Portal에서 가상 네트워크를 선택합니다 `CoreServicesVnet` .
...
이 작업에서는 서로 다른 가상 네트워크의 가상 머신 간의 연결을 다시 테스트합니다. 
...
>**알고 계셨나요?** 연결을 검사 방법에는 여러 가지가 있습니다. 이 작업에서는 실행 명령을** 사용합니다**. Network Watcher를 계속 사용할 수도 있습니다. 또는 원격 데스크톱 커넥트 사용하여 [](https://learn.microsoft.com/azure/virtual-machines/windows/connect-rdp#connect-to-the-virtual-machine) 가상 머신에 액세스할 수 있습니다. 연결되면 테스트 연결을** 사용합니다**. 시간이 지남에 따라 RDP에 시도해 보세요. 

1. 가상 머신으로 `ManufacturingVM` 전환합니다.

1. **작업** 블레이드에서 실행 명령** 블레이드를 **선택합니다.
...
1. RunPowerShellScript**를 **선택하고 **Test-Net커넥트ion** 명령을 실행합니다. CoreServicesVM**의 **개인 IP 주소를 사용해야 합니다.
    ...
    ```
1. 스크립트가 시간 초과되는 데 몇 분 정도 걸릴 수 있습니다. 페이지 위쪽에는 진행 중인 정보 메시지 *스크립트 실행이 표시됩니다.*

   
1. 피어링이 구성되었으므로 테스트 연결이 성공해야 합니다. 이 그래픽의 컴퓨터 이름과 원격 주소는 다를 수 있습니다. 
   
   ![Test-Net커넥트ion이 있는 PowerShell 창이 성공했습니다.](../media/az104-lab05-success.png)

## 작업 6: 사용자 지정 경로 만들기 

이 작업에서는 경계 서브넷과 내부 코어 서비스 서브넷 간의 네트워크 트래픽을 제어하려고 합니다. 가상 네트워크 어플라이언스 핵심 서비스 서브넷에 설치되며 모든 트래픽이 해당 서브넷에 라우팅되어야 합니다. 

1. 을 검색하여 `CoreServicesVnet`선택합니다.

1. 서브넷을 선택한 다음 + 만들기를** 선택합니다****.** 변경 내용을 **저장**합니다. 
...
1. Azure Portal에서 검색하여 선택한 `Route tables`다음 만들기**를 선택합니다**. 
...
1. 경로 테이블이 배포된 후 리소스**로 이동을 선택합니다**.

1. 경로를** 선택한 **다음 **+ 추가**합니다. 향후 NVA에서 CoreServices 가상 네트워크로의 경로를 만듭니다. 
...
1. 경로가 완료되면 +추가**를 선택합니다**. 마지막으로 수행할 작업은 경로를 서브넷과 연결하는 것입니다.

1. 서브넷을 선택한 **다음 **연결**합니다**. 구성을 완료합니다.
...
>**참고**: DMZ에서 새 NVA로 트래픽을 전송하는 사용자 정의 경로를 만들었습니다.  
...
+ 시스템 정의 경로는 가상 네트워크의 각 서브넷에 대해 자동으로 만들어집니다. 사용자 정의 경로는 기본 시스템 경로를 재정의하거나 기본 시스템 경로에 추가합니다. 
+ Azure Network Watcher는 Azure IaaS 리소스에 대한 메트릭 및 로그를 모니터링, 진단 및 볼 수 있는 도구 모음을 제공합니다.

## 자기 주도적 학습을 통해 자세히 알아보기

+ [Azure 가상 네트워크에 서비스를 배포하고 가상 네트워크 피어링을 사용하여 통합합니다](https://learn.microsoft.com/en-us/training/modules/integrate-vnets-with-vnet-peering/). 가상 네트워크 피어링을 사용하여 안전하며 복잡성을 최소로 하는 방법으로 가상 네트워크 간에 통신을 사용하도록 설정합니다.
+ [경로를](https://learn.microsoft.com/training/modules/control-network-traffic-flow-with-routes/) 사용하여 Azure 배포에서 트래픽 흐름을 관리하고 제어합니다. 사용자 지정 경로를 구현하여 Azure 가상 네트워크 트래픽을 제어하는 방법을 알아봅니다.

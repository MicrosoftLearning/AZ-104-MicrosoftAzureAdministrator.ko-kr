...
1. 가상 머신 페이지에서 **만들기**를 선택한 다음 **Virtual machine**을 선택합니다.
...
1. 기본 사항 탭에서 다음 정보를 사용하여 양식을 작성한 후 **다음: 디스크 >** 를 클릭합니다. 지정되지 않은 설정은 기본값을 그대로 둡니다.
    ...
    | 가용성 옵션 | 인프라 중복 필요 없음 |
    | 보안 유형 | **Standard** |
    | 이미지(모든 이미지 참조) | **Windows Server 2019 Datacenter - x64 Gen2**(다른 선택 사항 확인) |
    | 크기 | **Standard_DS2_v3** |
    | 사용자 이름 | `localadmin` | 
    | 암호 | **복잡한 암호 제공** |
    | 퍼블릭 인바운드 포트 | **없음** |
...
1. 다음 정보를 사용하여 가상 네트워크를 구성한 다음 **OK**를 선택합니다. 필요한 경우 기존 정보를 제거하거나 바꿉니다.
    ...
    | 설정 | 값 | 
    | --- | --- |
    | 속성 | `CoreServicesVnet`(만들기 또는 편집) |
    | 주소 범위 | `10.0.0.0/16`  |
    | 서브넷 이름 | `Core` | 
    | 서브넷 주소 범위 | `10.0.0.0/24` |

1. **모니터링** 탭을 선택합니다. 부팅 진단에 대해 **사용 안 함**을 선택합니다.
...
1. CoreServicesVnet의 피어링에서 **+ 추가**를 선택합니다. 지정되지 않은 경우 기본값을 사용합니다. 
    ...
    | 가상 네트워크    | **ManufacturingVnet(az104-rg5)**  |
    | 'CoreServicesVnet'이 'ManufacturingVnet'에 액세스하도록 허용            | 선택됨(기본값) |
    | 'CoreServicesVnet'이 'ManufacturingVnet'에서 전달된 트래픽을 수신하도록 허용 | 선택됨 |
    | 피어링 링크 이름                             | `CoreServicesVnet-to-ManufacturingVnet` |
    | 'ManufacturingVnet'이 'CoreServicesVnet'에 액세스하도록 허용  | 선택됨(기본값) |
    | ‘ManufacturingVnet’이 ‘CoreServicesVnet’에서 전달된 트래픽을 수신하도록 허용 | 선택됨  |
...
이 작업에서는 경계 서브넷과 내부 핵심 서비스 서브넷 간의 네트워크 트래픽을 제어하려고 합니다. 가상 네트워크 어플라이언스가 경계 서브넷에 설치되며 모든 트래픽은 해당 어플라이언스에 라우팅되어야 합니다. 
...
1. Azure Portal에서 `Route tables` 검색 및 선택 후, **+ 만들기**를 선택합니다.

1. 다음 세부 정보를 입력한 다음 **검토 + 만들기**를 선택하고 **만들기**를 선택합니다. 
...
1. **서브넷**을 선택한 다음 **+ 연결**을 선택합니다. 구성을 완료합니다.
    ...
    | 설정 | 값 | 
    | --- | --- |
    | 가상 네트워크 | **CoreServicesVnet(az104-rg5)** |
    | 서브넷 | **핵심**
           |    

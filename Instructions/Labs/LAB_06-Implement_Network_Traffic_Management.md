...
1. 다음 설정을 사용하여 피어링을 추가하고(다른 설정은 기본값으로 유지) **추가**를 클릭합니다.
    ...
    | 'az104-06-vnet01'이 피어링된 가상 네트워크에 액세스하도록 허용 | **확인란이 선택되어 있는지 확인합니다(기본값)** |
    | 'az104-06-vnet01'의 게이트웨이가 피어링된 가상 네트워크로 트래픽을 전달하도록 허용 | **확인란이 선택되어 있는지 확인합니다.** |
    | 원격 가상 네트워크: 피어링 링크 이름 | **az104-06-vnet2_to_az104-06-vnet01** |
    | 가상 네트워크 배포 모델 | **리소스 관리자** |
    | 리소스 ID를 알고 있음 | 사용 |
    | 리소스 ID | 이 작업 앞부분에서 기록한 **az104-06-vnet2** 의 resourceID 매개 변수 값입니다. |
    | az104-06-vnet2가 az104-06-vnet01에 액세스하도록 허용 | **확인란이 선택되어 있는지 확인합니다(기본값)** |
    | az104-06-vnet2가 az104-06-vnet01에서 전달된 트래픽을 수신하도록 허용 | **확인란이 선택되어 있는지 확인합니다.** |
...
1. 다음 설정을 사용하여 피어링을 추가하고(다른 설정은 기본값으로 유지) **추가**를 클릭합니다.
    ...
    | 가상 네트워크 배포 모델 | **리소스 관리자** |
    | 리소스 ID를 알고 있음 | 사용 |
    | 리소스 ID | 이 작업 앞부분에서 기록한 **az104-06-vnet3** 의 resourceID 매개 변수 값입니다. |
    | az104-06-vnet3이 az104-06-vnet01에 액세스하도록 허용 | **확인란이 선택되어 있는지 확인합니다(기본값).** |
    | az104-06-vnet3이 az104-06-vnet01에서 전달된 트래픽을 수신하도록 허용 | **확인란이 선택되어 있는지 확인합니다.** |

...
1. 다음 설정을 사용하여 피어링을 추가하고(다른 설정은 기본값으로 유지) **추가**를 클릭합니다.
    ...
    | 'az104-06-vnet01'이 피어된 가상 네트워크에 액세스하도록 허용 | **상자가 검사(기본값)** |
    | 'az104-06-vnet01'의 게이트웨이가 피어된 가상 네트워크로 트래픽을 전달하도록 허용 | **상자가 검사 확인** |
    | 원격 가상 네트워크: 피어링 링크 이름 | **az104-06-vnet2_to_az104-06-vnet01** |
    | 가상 네트워크 배포 모델 | **리소스 관리자** |
    | 리소스 ID를 알고 있음 | 사용 |
    | 리소스 ID | 이 작업의 앞부분에서 기록한 az104-06-vnet2**의 **resourceID 매개 변수 값입니다. |
    | az104-06-vnet2가 az104-06-vnet01에 액세스하도록 허용 | **상자가 검사(기본값)** |
    | az104-06-vnet2가 az104-06-vnet01에서 전달된 트래픽을 수신하도록 허용 | **상자가 검사 확인** |
...
1. 다음 설정을 사용하여 피어링을 추가하고(다른 설정은 기본값으로 유지) **추가**를 클릭합니다.
    ...
    | 가상 네트워크 배포 모델 | **리소스 관리자** |
    | 리소스 ID를 알고 있음 | 사용 |
    | 리소스 ID | 이 작업의 앞부분에서 기록한 az104-06-vnet3**의 **resourceID 매개 변수 값입니다. |
    | az104-06-vnet3이 az104-06-vnet01에 액세스하도록 허용 | **상자가 검사(기본값)** |
    | az104-06-vnet3이 az104-06-vnet01에서 전달된 트래픽을 수신하도록 허용 | **상자가 검사 확인** |
...
1. **진단 테스트 실행**을 클릭하고 연결 확인 결과가 반환될 때까지 기다립니다. 상태가 **실패**입니다.
...
1. **Network Watcher - 연결 문제 해결** 블레이드에서 다음 설정을 사용합니다(다른 설정은 기본값으로 유지).
...
1. **진단 테스트 실행**을 클릭하고 연결 확인 결과가 반환될 때까지 기다립니다. 상태가 **성공**인지 확인합니다. 네트워크 경로를 검토하고 트래픽이 **10.60.0.4**를 통해 라우팅되고 **az104-06-nic0** 네트워크 어댑터에 할당되어있습니다. 상태가 **실패**이면 중지한 다음 az104-06-vm0을 시작해야 합니다.

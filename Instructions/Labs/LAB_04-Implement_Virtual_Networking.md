...
1. **+ 서브넷 추가**를 선택합니다. 각 서브넷의 이름과 주소 정보를 입력합니다. 각각의 새 서브넷에 대해 **추가**를 선택해야 합니다. 

    >**참고:** 다른 서브넷을 만들기 전이나 후에 기본 서브넷을 삭제해야 합니다.
...
1. 파일을 다시 읽어보고 모든 내용이 올바른지 확인합니다. 리소스 이름 및 IP 주소에 아키텍처 다이어그램을 사용합니다. 
...
1. 포털에서 `Deploy a custom template`를 검색하여 선택합니다.

1. **편집기에서 사용자 고유의 템플릿 만들기**를 선택한 다음 **파일 로드**를 선택합니다.

1. 제조 변경 내용이 포함된 **template.json** 파일을 선택한 다음 **저장**을 선택합니다.

1. **매개 변수 편집**을 선택한 다음 **로드 파일**을 선택합니다.

1. 제조 변경 사항이 포함된 **parameters.json** 파일을 선택한 다음 **저장**을 선택합니다.

1. 리소스 그룹인 **az104-rg4**가 선택되어 있는지 확인합니다. 
...
>**참고:** Azure Portal 메뉴(왼쪽 상단의 아이콘)를 사용하여 이 리소스를 찾을 수도 있습니다. **리소스 만들기**를 선택한 다음, **네트워킹** 블레이드에서 **네트워크 보안 그룹**을 선택합니다. 
...
1. **AllowInternetOutBound** 규칙을 확인합니다. 또한 규칙은 삭제할 수 없으며 우선 순위는 65001입니다.
...
1. **DNS 관리** 블레이드를 확장하고 **레코드 집합**을 선택합니다. **+추가**를 클릭합니다. 
...
1. **추가**를 선택하고 도메인에 **www**라는 이름의 A 레코드 집합이 있는지 확인합니다.

1. 명령 프롬프트를 열고 다음 명령을 실행합니다. 도메인 이름을 변경한 경우 조정합니다. 
...
1. **DNS 관리** 블레이드를 확장한 다음 **가상 네트워크 링크**를 선택합니다. 링크를 구성합니다. 

...
1. **다음: IP 주소**를 클릭합니다. **시작 주소**는 **10.40.0.0**입니다. **주소 공간 크기**는 **/20**입니다. 

1. + 서브넷** 추가를 클릭합니다**. 기존 기본** 서브넷을 **삭제합니다. 다음 값을 입력한 다음 추가**를 클릭합니다**. 
...
1. Cloud Shell 창의 도구 모음에서 **파일 업로드/다운로드** 아이콘을 클릭하고 드롭다운 메뉴에서 **업로드**를 클릭합니다. Cloud Shell 홈 디렉터리에 **\\Allfiles\\Labs\\04\\az104-04-vms-loop-template.json** 및 **\\Allfiles\\Labs\\04\\az104-04-vms-loop-parameters.json**을 업로드합니다.

    >**참고**: 각 파일을 별도로 업로드해야 합니다. 업로드한 후 **dir**을 사용하여 두 파일이 성공적으로 업로드되었는지 확인합니다.
...
이 작업에서는 Azure 가상 머신의 네트워크 인터페이스에 할당된 공용 및 개인 IP 주소에 대한 정적 할당을 구성합니다.
   ...
   >**참고**: 이 랩을 완료하려면 두 개의** 공용 IP 주소가 필요합니다**. 

1. Azure Portal에서 공용 IP 주소를** 검색하여 선택한 **다음+ 만들기**를 선택합니다**.

1. **리소스 그룹이** **az104-04-rg1**인지 확인합니다.

1. **구성 세부 정보**에서 이름이** az104-04-pip0**인지 **확인**합니다.
...
1. **구성 세부 정보**에서 이름이** az104-04-pip1**인지 **확인**합니다.
...
1. 공용 IP 주소 연결을 선택하고 **공용 IP 주소** 드롭다운에서 **az104-04-pip0**을 선택합니다**.**

    >**참고:** 오류가 *발생하면 기본 이름이 이미 사용* 중이므로 알려진 문제입니다. 공용 IP 주소를 찾아서 NIC에 별도로 연결해야 합니다.
    >
    > + 공용 IP 주소로 **이동**
    > + az104-04-pip0 클릭 ****
    > + 개요** 창에서 **IP 연결 클릭 ****
    > + 리소스 유형을** 네트워크 인터페이스로 **설정 ****
    > + 네트워크 인터페이스**를 **az104-04-nic0으로 설정 ****
    > + az104-04-pip1** 및 **az104-04-nic1에 대해 **반복**

1. **저장**을 선택합니다.

1. az104-04-vnet1** 블레이드로 다시 **이동합니다.
...
1. 공용 IP 주소 연결을 선택하고 **공용 IP 주소** 드롭다운에서 **az104-04-pip1**을 선택합니다**.**
...
1. Cloud Shell 창에서 다음을 실행하여 새로 만든 DNS 영역에 설정된 az104-04-vm0** DNS 레코드의 **외부 이름 확인을 테스트합니다(자리 표시자를 `[Name server 1]` 이 태스크의 **앞부분에서 적어 둔 이름 서버 1**의 `[domain name]` 이름으로 바꾸고 자리 표시자를 DNS의 이름으로 바꾸기기본 이 태스크의 앞부분에서 만든 이름).
...
1. Cloud Shell 창에서 다음을 실행하여 새로 만든 DNS 영역에 설정된 az104-04-vm1** DNS 레코드의 **외부 이름 확인을 테스트합니다(자리 표시자를 `[Name server 1]` 이 작업 앞부**분에서 적어 둔 이름 서버 1**의 `[domain name]` 이름으로 바꾸고 자리 표시자를 DNS 이름으로 바꿉기본 이 태스크의 앞부분에서 만든 이름).

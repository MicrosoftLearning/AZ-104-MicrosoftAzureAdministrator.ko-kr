...
1. **공용 IP 주소 연결을** 선택하고 **공용 IP 주소** 드롭다운에서 **az104-04-pip1을** 선택합니다.
...
1. Cloud Shell 창에서 다음을 실행하여 새로 만든 DNS 영역에 설정된 **az104-04-vm0** DNS 레코드의 외부 이름 확인을 테스트합니다(자리 표시자를 `[Name server 1]` 이 작업 앞부분에서 적어 둔 **이름 서버 1**의 `[domain name]` 이름으로 바꾸고 자리 표시자를 이 작업 앞부분에서 만든 DNS 도메인의 이름으로 바꾸세요).
...
1. Cloud Shell 창에서 다음을 실행하여 새로 만든 DNS 영역에 설정된 **az104-04-vm1** DNS 레코드의 외부 이름 확인을 테스트합니다(자리 표시자를 `[Name server 1]` 이 작업 앞부분에서 적어 둔 **이름 서버 1**의 `[domain name]` 이름으로 바꾸고 자리 표시자를 이 작업 앞부분에서 만든 DNS 도메인의 이름으로 바꾸세요).

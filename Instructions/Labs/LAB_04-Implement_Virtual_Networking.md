...
이 랩은 가상 네트워킹에 중점을 둔 세 개의 랩 중 첫 번째 랩입니다. 이 랩에서는 가상 네트워킹 및 서브넷의 기본 사항에 대해 알아봅니다. 네트워크 보안 그룹 및 애플리케이션 보안 그룹을 사용하여 네트워크를 보호하는 방법을 알아봅니다. DNS 영역 및 레코드에 대해서도 알아봅니다. 
...
글로벌 조직은 가상 네트워크를 구현할 계획입니다. 즉각적인 목표는 모든 기존 리소스를 수용하는 것입니다. 그러나 조직은 성장 단계에 있으며 성장을 위한 추가 용량이 있는지 확인하려고 합니다.

**CoreServicesVnet** 가상 네트워크는 가장 많은 수의 리소스를 사용합니다. 많은 양의 증가가 예상되므로 이 가상 네트워크에는 대용량 주소 공간이 필요합니다.

ManufacturingVnet** 가상 네트워크에는 **제조 시설의 운영을 위한 시스템이 포함되어 있습니다. 조직은 시스템에서 데이터를 검색할 수 있도록 많은 수의 내부 연결된 디바이스를 예상하고 있습니다. 
...
조직은 핵심 서비스에 대해 많은 성장을 계획하고 있습니다. 이 작업에서는 기존 리소스 및 계획된 증가를 수용하기 위해 가상 네트워크 및 연결된 서브넷을 만듭니다. 이 작업에서는 Azure Portal을 사용합니다. 
...
1. **CoreServicesVnet**에 대한 기본 사항 탭을 완료합니다.  
...
1. IP 주소 탭으로 **** 이동합니다.
    ...
    | IPv4 주소 공간 | `10.20.0.0/16` (항목을 구분) |
...
1. **+ 서브넷 추가**를 선택합니다. 각 서브넷의 이름 및 주소 정보를 완료합니다. 새 서브넷마다 추가**를 선택**해야 합니다. 
    ...
    >**참고:** 모든 가상 네트워크에는 하나 이상의 서브넷이 있어야 합니다. 5개의 IP 주소는 항상 예약되므로 계획에서 고려해야 합니다. 
...
1. 주소 공간 및 서브넷을 **확인하는 데 1분 정도 걸릴 수 있습니다 **.**** 설정** 블레이드에서 **다른 선택 항목을 확인합니다. 
...
1. 계속하기 전에 template.json** 파일이 있는지 확인합니다**. 이 템플릿을 사용하여 다음 작업에서 ManufacturingVnet을 만듭니다. 
 
## 작업 2: 템플릿을 사용하여 가상 네트워크 및 서브넷 만들기

이 작업에서는 ManufacturingVnet 가상 네트워크 및 연결된 서브넷을 만듭니다. 이 조직은 제조 사무소의 성장을 예상하므로 서브넷의 크기는 예상 증가에 적합합니다. 이 작업의 경우 템플릿을 사용하여 리소스를 만듭니다. 

1. **이전 작업에서 내보낸 template.json** 파일을 찾습니다. 다운로드** 폴더에 **있어야 합니다.

1. 선택한 편집기를 사용하여 파일을 편집합니다. 많은 편집기에서 *모든 항목 기능을 변경합니다* . Visual Studio Code를 사용하는 경우 제한된 모드**가 아닌 신뢰할 수 있는 창**에서 **작업**하고 있는지 확인합니다. 세부 정보를 확인하려면 아키텍처 다이어그램을 참조하세요. 
...
1. 선택한 편집기를 사용하여 파일을 편집합니다.

1. CoreServicesVnet**의 **한 항목을 .로 바`ManufacturingVnet`꿉니다.
...
1. 템플릿이 배포되기를 기다린 다음(포털에서) 제조 가상 네트워크 및 서브넷이 생성되었는지 확인합니다.

>**참고:** 두 번 이상 배포해야 하는 경우 일부 리소스가 성공적으로 완료되고 배포가 실패하는 것을 확인할 수 있습니다. 이러한 리소스를 수동으로 제거하고 다시 시도할 수 있습니다. 
   
## 작업 3: 애플리케이션 보안 그룹과 네트워크 보안 그룹 간의 통신 만들기 및 구성

이 작업에서는 애플리케이션 보안 그룹 및 네트워크 보안 그룹을 만듭니다. NSG에는 ASG의 트래픽을 허용하는 인바운드 보안 규칙이 있습니다. 또한 NSG에는 인터넷에 대한 액세스를 거부하는 아웃바운드 규칙이 있습니다. 

### ASG(애플리케이션 보안 그룹) 만들기

1. Azure Portal에서 검색하여 선택합니다 `Application security groups`.
...
1. Azure Portal에서 검색하여 선택합니다 `Network security groups`.
...
1. NSG가 배포된 후 리소스**로 이동을 클릭합니다**.

1. 설정** 아래에서 **서브넷을 클릭한 **다음 **연결**합니다**.
...
1. NSG 작업을 계속합니다. **설정** 영역에서 인바운드 보안 규칙을** 선택합니다**.
...
1. 인바운드 NSG 규칙을 만든 후 아웃바운드 보안 규칙을** 선택합니다**. 

1. **AllowInternetOutboundRule** 규칙을 확인합니다. 또한 규칙을 삭제할 수 없으며 우선 순위는 65001입니다.

1. + 추가**를 선택한 **다음 인터넷에 대한 액세스를 거부하는 아웃바운드 규칙을 구성합니다. 완료되면 추가**를 선택합니다**.
...
공용에서 호스트 이름을 확인하도록 Azure DNS를 구성할 수 있습니다기본. 예를 들어 do기본 이름 등록 기관에서 contoso.xyz do기본 이름을 구입한 경우 할 일기본 호스트 `contoso.com` 하고 웹 서버 또는 웹앱의 IP 주소로 www.contoso.xyz 확인하도록 Azure DNS를 구성할 수 있습니다.

1. 포털에서 검색하여 선택합니다 `DNS zones`.
...
1. 기본 사항 탭을 **구성합니다** .
    ...
    | 이름 | `contoso.com` (예약된 경우 이름을 조정) |
    | 지역 |**미국** 동부(정보 아이콘 검토) |

1. 만들기 검토를 선택한 다음 **만들기**** 를 선택합니다**.
   
1. DNS 영역이 배포되기를 기다린 다음 리소스**로 이동을 선택합니다**.

1. 개요** 블레이드에서 **영역에 할당된 4개의 Azure DNS 이름 서버의 이름을 확인합니다. **이름 서버 주소 중 하나를 복사** 합니다. 이는 다음 단계에서 필요합니다. 
...
1. 확인을** 선택하고 **contoso.com** www**라는 **A 레코드 집합이 있는지 확인**합니다.
...
1. 명령 프롬프트를 열고 다음 명령을 실행합니다.
   ...
   ```
1. www.contoso.com 호스트 이름이 제공한 IP 주소로 확인되는지 확인합니다. 이렇게 하면 이름 확인이 제대로 작동하는지 확인합니다.

### 프라이빗 DNS 영역 구성

프라이빗 DNS 영역은 가상 네트워크 내에서 이름 확인 서비스를 제공합니다. 프라이빗 DNS 영역은 연결된 가상 네트워크에서만 액세스할 수 있으며 인터넷에서 액세스할 수 없습니다. 

1. 포털에서 검색하여 선택합니다 `Private dns zones`.

1. **+ 만들기**를 선택합니다.

1. **프라이빗 DNS 영역 만들기의 기본** 사항 탭에서 아래 표에 나열된 대로 정보를 입력합니다.
...
1. 개요** 블레이드에는 **이름 서버 레코드가 없습니다. 

1. + 가상 네트워크 링크를 선택한 **다음+ 추가**를 선택합니다**.** 
...
1. 확인을** 선택하고 **링크가 생성되기를 기다립니다. 

1. **개요** 블레이드에서 + 레코드 집합**을 선택합니다**. 이제 프라이빗 이름 확인 지원이 필요한 각 가상 머신에 대한 레코드를 추가합니다.
...
+ Azure DNS는 이름 확인을 제공하는 DNS 도메인용 호스팅 서비스입니다. 공용에서 호스트 이름을 확인하도록 Azure DNS를 구성할 수 있습니다기본.  프라이빗 DNS 영역을 사용하여 Azure 가상 네트워크의 VM(가상 머신)에 DNS 이름을 할당할 수도 있습니다.

## 자기 주도적 학습을 통해 자세히 알아보기

+ [Azure Virtual Network 소개](https://learn.microsoft.com/training/modules/introduction-to-azure-virtual-networks/). 가상 네트워크, 공용 및 개인 IP, DNS, 가상 네트워크 피어링, 라우팅 및 Azure Virtual NAT와 같은 핵심 Azure 네트워킹 인프라를 디자인하고 구현합니다.
+ [IP 주소 지정 체계](https://learn.microsoft.com/training/modules/design-ip-addressing-for-azure/)를 디자인합니다. Azure 및 온-프레미스 가상 네트워크의 프라이빗 및 공용 IP 주소 지정 기능을 식별합니다.
+ [네트워크 보안 그룹 및 서비스 엔드포인트를 사용하여 Azure 리소스에 대한 액세스를 보호하고 격리합니다](https://learn.microsoft.com/training/modules/secure-and-isolate-with-nsg-and-service-endpoints/). 네트워크 보안 그룹과 서비스 엔드포인트를 통해 무단 네트워크 액세스로부터 가상 머신과 Azure 서비스를 보호할 수 있습니다.
+ [Azure DNS](https://learn.microsoft.com/training/modules/host-domain-azure-dns/)에서 할 일기본 호스트합니다. 도메인 이름에 대한 DNS 영역을 만듭니다. DNS 레코드를 만들어 do기본 IP 주소에 매핑합니다. 도메인 이름이 웹 서버로 확인되는지 테스트합니다.

...
1. 다음 설정을 지정하여 할당의 **기본** 속성을 구성합니다(다른 설정은 기본값으로 유지).
    ...
    >**참고**: 선택한 정책 이름이 **할당 이름**에 자동으로 채워지지만, 할당 이름을 변경할 수 있습니다. 설명**은 **선택 사항입니다. **할당 기준** 은 할당을 만드는 사용자 이름에 따라 자동으로 채워집니다. 
...
1. 개요 블레이드에서 **Infra** 값을 **가진 태그 **역할이** 리소스에 자동으로 할당**되었습니다.

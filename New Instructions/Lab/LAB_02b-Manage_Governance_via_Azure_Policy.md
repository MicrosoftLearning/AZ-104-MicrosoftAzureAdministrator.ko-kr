...
1. **작성** 섹션에서 **정의**를 클릭합니다. 잠시 시간을 내어 사용할 수 있는 기본 제공 정책 정의[ 목록을 ](https://learn.microsoft.com/azure/governance/policy/samples/built-in-policies)살펴보세요. 정의를 검색할 수도 있습니다.
...
1. **줄임표 단추를 클릭하고 다음 값을 선택하여 범위를** 지정합니다. 완료되면 [선택 **]을 클릭합니다**. 
    ...
    >**참고**: 범위는 정책 할당이 적용되는 리소스 또는 리소스 그룹을 결정합니다. 관리 그룹, 구독 또는 리소스 그룹 수준에서 정책을 할당할 수 있습니다. 개별 구독, 리소스 그룹 또는 리소스와 같은 제외를 지정할 수도 있습니다.
...
1. 다음 설정을 지정하여 할당의 **기본** 속성을 구성합니다(다른 설정은 기본값으로 유지).
    ...
    >**참고**: 선택한 정책 이름이 **할당 이름**에 자동으로 채워지지만, 할당 이름을 변경할 수 있습니다. 설명**은 **선택 사항입니다. 
...
1. 추가를 선택한 **다음, 할당의 기본 속성을 다시 기본 **구성합니다**.**
    ...
    | 설정 | 값 |
    | --- | --- |
    | 할당 이름 | `Inherit the Role tag and its Infra value from the resource group if missing` |
    | 설명 | `Inherit the Role tag and its Infra value from the resource group if missing` |
    | 정책 적용 | 사용 |
...
1. 태그 블레이드에서 **Infra** 값을 **가진 태그 **역할이** 리소스에 자동으로 할당**되었습니다.

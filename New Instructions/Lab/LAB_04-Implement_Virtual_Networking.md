...
1. 가상 네트워크 페이지에서 만들기를 선택하고 **기본 및** **IP 주소 탭을** 완료**합니다.** 
...
1. 다음 표의 정보를 사용하여 CoreServicesVnet 가상 네트워크를 만듭니다.  
    ...
    |              | 이름               | `CoreServicesVnet`     |
    |              | 지역             | (미국) **미국 동부**         |
    | IP 주소 | IPv4 주소 공간 | `10.20.0.0/16` (항목을 구분)    |
...
1. **인바운드 보안 규칙** 추가 블레이드에서 다음 정보를 사용하여 인바운드 포트 규칙을 추가한 다음 추가**를 선택합니다**.
...
1. NSG 규칙을 만든 후 잠시 시간을 내어 기본 **아웃바운드 보안 규칙을** 검토합니다.

...
+ 작업 3: 사용자 지정 RBAC 역할을 만들고 할당합니다. 
...
이 작업에서는 사용자 계정에 VM 기여자 역할을 할당합니다.  
...
### 지원 센터 사용자에 대한 사용자 지정 RBAC 역할 만들기

1. 포털에서 az104-mg1** 관리 그룹을 검색하여 선택합니다**.

1. **액세스 제어(IAM)** 블레이드를 선택한 다음 역할** 탭을 **선택합니다.

1. 액세스 확인 탭을 **선택한 다음 사용자 지정 역할** 만들기 상자에서 **추가**를 선택합니다**.**
...
### 새 역할을 테스트하는 데 사용할 지원 센터 사용자 계정을 ID로 지정하고 사용자 지정 역할을 할당합니다. 

1. Azure Portal에서 Microsoft Entra ID**를 검색하여 선택한 **다음, 사용자** 블레이드를 **선택합니다.

    >**참고**: 이 작업을 수행하려면 테스트용 사용자 계정이 필요합니다. 이 랩에서는 HelpDesk-user1**을 사용합니다**. 필요한 **경우 테스트 사용자를 식별하려면 잠시 시간을 내어 새 사용자를 추가** 하세요. 새 사용자를 만드는 경우 로그인할 때 암호를 설정해야 합니다. 

1. 계속하기 전에 테스트 계정의 **사용자 계정 이름이** 있는지 확인합니다. 포털에 로그인하려면 이 항목이 필요합니다. 아이콘을 사용하여 이 정보를 클립보드에 복사합니다. 

1. Azure Portal에서 az104-mg1** 관리 그룹으로 다시 **이동하여 세부 정보를 표시합니다.
...
1. **역할**을 선택하고 **다음**을 클릭합니다. 구성원 탭에서 **+ 구성원 **** 선택을 클릭하고 **사용자 계정 **HelpDesk-user1**을 선택합니다**.**  

1. 검토를 선택하고 **두 번 할당** 합니다.

    >**참고:** 이 시점에서 지원 티켓을 만들 수 있는 사용자 지정 권한이 있는 지원 센터 사용자 계정이 있습니다. 다음 단계는 계정을 테스트하는 것입니다.
    
### 지원 센터 사용자 계정을 테스트하여 올바른 권한이 있는지 확인합니다.

1. **InPrivate** 브라우저 창을 열고 테스트 사용자 계정을 사용하여 Azure Portal에 `https://portal.azure.com` 로그인합니다. 암호를 업데이트하라는 메시지가 표시되면 사용자의 암호를 변경합니다.

    >**참고**: 사용자 이름을 입력하는 대신 클립보드의 콘텐츠를 붙여넣을 수 있습니다.

1. InPrivate** 브라우저 창의 **Azure Portal에서 리소스 그룹을** 검색하고 선택하여 **지원 센터 사용자가 모든 리소스 그룹을 볼 수 있는지 확인합니다.

1. InPrivate** 브라우저 창의 **Azure Portal에서 모든 리소스를** 검색하고 선택하여 **지원 센터 사용자가 리소스를 볼 수 없는지 확인합니다.
...
1. 사용자 지정 역할 테스트를 완료하고 지원 티켓을 만드는 방법을 검토했습니다. 

## 작업 4: 활동 로그를 사용하여 역할 할당 모니터링

이 작업에서는 활동 로그를 확인하여 새 역할을 만든 사람이 있는지 확인합니다. 

1. **az104-mg1** 리소스로 돌아가서 활동 로그**를 선택합니다**.

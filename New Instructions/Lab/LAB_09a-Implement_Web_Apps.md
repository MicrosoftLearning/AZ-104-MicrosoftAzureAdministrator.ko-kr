...
조직에서는 회사 웹 사이트를 호스트하기 위해 Azure Web Apps에 관심이 있습니다. 웹 사이트는 현재 온-프레미스 데이터 센터에서 호스트됩니다. 웹 사이트는 PHP 런타임 스택을 사용하여 Windows 서버에서 실행됩니다. 하드웨어의 수명이 거의 끝나가고 있으며 곧 교체해야 합니다. 조직에서는 Azure를 사용하여 웹 사이트를 호스트하여 하드웨어를 교체하지 않으려고 합니다. 
...
이 작업에서는 Azure 웹앱을 만듭니다. Azure 앱 Services는 웹, 모바일 및 기타 웹 기반 애플리케이션을 위한 PAAS(Platform As a Service) 솔루션입니다. Azure 웹앱은 PHP, Java 및 .NET과 같은 대부분의 런타임 환경을 호스팅하는 Azure 앱 서비스의 일부입니다. 선택한 SKU는 웹앱으로 수신하는 컴퓨팅, 스토리지 및 기능의 양을 결정합니다.
...
이 작업에서는 웹앱 배포 설정을 구성합니다. 배포 설정을 사용하면 선택한 리포지토리에서 또는 FTPS 자격 증명 및 기타 자동화를 사용하여 지속적으로 배포할 수 있습니다. 이렇게 하면 앱 서비스에 최신 버전의 애플리케이션이 있습니다.

1. 스테이징 슬롯에서 배포 센터를** 선택한 **다음, 설정** 선택합니다**.

    >**참고:** 스테이징 슬롯 블레이드(프로덕션 슬롯 대신)에 있는지 확인합니다.
    
**원본** 드롭다운 목록을 1.In 외부 Git을** 선택합니다**.

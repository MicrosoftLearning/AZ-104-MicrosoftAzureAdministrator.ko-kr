...
1. 필요한 경우 Allfiles Lab11\\az104-11-vm-template.json** 및\\** Allfiles\\Labs\\11\\az104-11-vm-parameters.json** 랩 파일을 컴퓨터에 다운로드\\**합니다.\\
...
1. az104-rg11 리소스 그룹에 대한 **상자를 선택한 다음 적용을 선택합니다****.** 이 경고는 리소스 그룹의 모든 가상 머신에 적용됩니다. 또는 특정 컴퓨터를 하나만 지정할 수 있습니다. 

1. 조건 탭을 **선택한 다음 모든 신호** 보기 링크를 선택합니다**.**

1. Virtual Machine(Virtual Machines)을 검색하여 삭제를** 선택합니다**. 다른 기본 제공 신호를 확인합니다. **적용**을 선택합니다.

1. 모든 형식의 경고를 수신하고 **경고 논리** 설정을 기본값인 **모두 선택**으로 둡니다.

1. 다음 섹션에 대한 **경고 규칙 만들기** 창을 열어 둡니다.

## 작업 3: 전자 메일 경고 작업 추가

이 작업에서 경고가 트리거되면 이메일 알림이 운영 팀에 전송됩니다. 
...
1. **검토 + 만들기**를 선택하여 입력의 유효성을 검사한 다음, **만들기**를 선택합니다.
    ...
    >**참고:** 작업 그룹에 추가되었다는 전자 메일 알림을 받아야 합니다. 
...
이 작업에서는 기본 테넌트 기간 동안 알림을 표시하지 않는 경고 규칙을 만듭니다. 
...
1. 기본적으로 규칙은 사용하지 않도록 설정하지 않는 한 항상 작동합니다. 야간 계획된 기본 동안 알림을 표시하지 않는 규칙을 정의합니다.

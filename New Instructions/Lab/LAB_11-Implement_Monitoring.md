...
이 작업에서 경고가 트리거되면 운영 팀에 전송되는 이메일 알림입니다. 
...
1. **경고 규칙** 만들기 창에서 다음: 세부 정보** 단추를 선택하고 **각 설정에 대해 다음 값을 입력합니다.
...
1. 포털에서 가상 머신**을 검색하고 선택합니다**.

1. az104-vm0** 가상 머신에 대한 **확인란을 선택합니다.

1. 메뉴 모음에서 **삭제**를 선택합니다.

1. 강제 삭제** 적용 확인란을 **선택합니다. 삭제를 승인하려면 확인란을 선택하고 삭제**를 선택합니다**. 
...
1. 쿼리 창에서 **가상 머신 **(왼쪽 창)을 선택합니다**.** 필요한 경우 시작 화면을 닫습니다. 

1. 사용 가능한 쿼리를 Reveiw합니다. **Count 하트비트 쿼리를 **실행**합니다(쿼리를 마우스로 가리키기**).
...
1. 시간이 지남에 따라 다른 쿼리를 검토하고 실행합니다.
    ...
    >**참고**: uou는 쿼리에 따라 경고 규칙을 구성할 수 있습니다. 
...
+ 작업 그룹에는 경고를 알려야 하는 개인이 포함되므로 조치를 취할 수 있습니다. 

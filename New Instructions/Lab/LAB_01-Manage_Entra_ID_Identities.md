...
1. **Azure Portal** - `https://portal.azure.com`에 로그인합니다.
    ...
    >**참고:** Azure Portal은 모든 랩에서 사용됩니다. Azure를 처음으로 사용하는 경우 검색하여 선택합니다 `Quickstart Center`. Azure** Portal 비디오에서 시작하기를 **시청하려면 몇 분 정도 걸립니다. 이전에 포털을 사용한 적이 있더라도 내부 탐색 및 사용자 지정에 대한 몇 가지 팁과 요령을 찾을 수 있습니다. 
...
1. Microsoft Entra ID는 Azure의 클라우드 기반 ID 및 액세스 관리 솔루션입니다. 왼쪽 창에 나열된 일부 기능을 숙지하려면 몇 분 정도 걸립니다. 

   + **관리기본 단위**를 사용하면 사용자, 그룹 또는 디바이스를 단일 관리 가능한 단위로 그룹화할 수 있습니다.
   + **라이선스를** 사용하면 라이선스를 구매하고, 가지고 있는 라이선스를 관리하고, 사용자 및 그룹에 라이선스를 할당할 수 있습니다.
...
1. + 새 그룹을** 선택하고 **새 그룹을 만듭니다. 
    ...
    >**참고**: 멤버 자격 유형** 드롭다운의 **옵션을 확인합니다. 동적 멤버 자격에는 Entra ID Premium P1 또는 P2 라이선스가 필요합니다. 
...
1. **탑재된** 스토리지가 없는 경우 화면에서 고급 설정** 표시를 선택하고 **필요한 정보를 제공합니다. 완료되면 스토리지** 만들기를 선택합니다**.
    ...
    >**참고:** Cloud Shell을 사용할 때 스토리지 사용 및 파일 공유가 필요합니다. 
...
1. 화살표 키를 사용하여 명령 기록을 이동합니다. 탭 키를 사용하여 명령 및 매개 변수를 자동으로 완성합니다. 언제든지 cls를** 사용하여 **명령 창을 지웁니다.
